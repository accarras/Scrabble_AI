    ...
    def drawTiles(self, board):
        ...
        if missingTileCount > 0:
            for i in range(missingTileCount):
                if board.tileBag:
                    tile = board.tileBag.pop()
                    self.rack.append(tile)
                    #print(self.rack)
                    
    def heuristic(self, move, board, miniMax):
        ...
        moveScore = self.getMoveScore(move, board, board.bwOld) + (len(move[0])-len(move[2]))*2
        
        p2 = [player for player in board.players if player != self][0]
        #print(board)
        testP1 = self.testMove(move, board, False)

        p2Moves = p2.findMoves(board, 10)

        if p2Moves:
            p2MoveScores = [(p2Move, self.getMoveScore(p2Move, board, board.bwOld) + (len(p2Move[0])-len(p2Move[2]))*2) for p2Move in p2Moves]

            #print(sorted(p2MoveScores, key=itemgetter(1)))

            bestMoveP2 = sorted(p2MoveScores, key=itemgetter(1))[-1]
            #print(bestMoveP2, moveScore, move)
            #print(board)
            self.undoTest(testP1[1], testP1[2], testP1[3], testP1[4], testP1[5], testP1[6], testP1[7])

            if miniMax:
                return moveScore - bestMoveP2[1]
            else:
                return -(moveScore - bestMoveP2[1])
        else:
            return moveScore

    def testMove(self, move, board, shouldUndo=True):
        ...
        try:
            ...
            return False, board, oldEmpty, oldPoints, oldPlayer, oldRack, oldBoard, oldTiles
        else:
            if shouldUndo:
                self.undoTest(board, oldEmpty, oldPoints, oldPlayer, oldRack, oldBoard, oldTiles)
            
            if tryMove:
                return True, board, oldEmpty, oldPoints, oldPlayer, oldRack, oldBoard, oldTiles
            else:
                return False, board, oldEmpty, oldPoints, oldPlayer, oldRack, oldBoard, oldTiles

    def undoTest(self, board, oldEmpty, oldPoints, oldPlayer, oldRack, oldBoard, oldTiles):
        #Undo move needed to be in separate function so it could be delayed.
            ...
            while True:
                ...
                if letterBoardCoOrds:
                    ...
                    for coOrds in letterBoardCoOrds:
                        #Check letters in words as well as checking 
                        emptyLeft = 0
                        lookLeft = True
                                ...
                                if length == 2:
                                    if i == 0:
                                        if emptyLeft >= 1 and emptyRight >= 2:
                                            ...
                                            testMove = self.testMove(move, board, True)[0]
                                            if testMove:
                                                moves.append(move)
            ...
            while True:
                ...
                for word in words:
                    ...
                    if board.width % 2 == 1:
                        ...
                        testMoveR = self.testMove(moveR, board, True)[0]
                        testMoveD = self.testMove(moveD, board, True)[0]
                        if testMoveR:
                            moves.append(moveR)
                        if testMoveD:
                            moves.append(moveD)
...
##                #print(move)
    ...
    def miniMax(self, board, player, limit):
...
##            return self.heuristic(board), -1
        ...
        miniMax = (player == board.players[0])
        
        if miniMax:
            bestValue = -math.inf
            moves = player.findMoves(board, limit)
            #print(moves)
...
##                    print(move)
                ...
                value = self.heuristic(move, board, miniMax)
                if value >= bestValue:
                    bestValue = value
                    bestMove = move
